...
namespace DNet.DataAccess
{
    public class DbFactory
    {
        ...
        public static IDatabase CreateDataBase()
        {
            ...
            {
                IDatabase database;
                if (DBProviderName.IndexOf(".SqlClient") != -1)
                {
                    database = new SqlServerDataBase(DBConnectionString);
                }
                else if (DBProviderName.IndexOf(".MySqlClient") != -1)
                {
                    database = new MySqlDataBase(DBConnectionString);
                }
                else
                {
                    database = new OracleDataBase(DBConnectionString);
                }
                return database;
            }
            catch(Exception ex)
        ...
        public static IDatabase CreateDataBase(ConnectionStringSettings settings)
        {
            ...
            {
                IDatabase database;
                if (settings.ProviderName.IndexOf(".SqlClient") != -1)
                {
                    database = new SqlServerDataBase(settings.ConnectionString);
                }
                else if (settings.ProviderName.IndexOf(".MySqlClient") != -1)
                {
                    database = new MySqlDataBase(settings.ConnectionString);
                }
                else
                {
                    database = new OracleDataBase(settings.ConnectionString);
                }
                return database;
            }
            catch

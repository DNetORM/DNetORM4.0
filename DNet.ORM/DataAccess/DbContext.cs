...
namespace DNet.DataAccess
{
    ...
    public class DbContext : IDisposable
    {
        ...
        BindingFlags BindFlag = BindingFlags.Public | BindingFlags.IgnoreCase | BindingFlags.Instance;
        ...
        protected int InsertT<T>(T entity)
        {
            try
            {
                ...
                {
                    if (this.DataBase.DBType == DataBaseType.MySql)
                    {
                        insertSql.Append(insertFields.ToString().TrimEnd(','));
                        insertSql.Append(" ) VALUES ( ");
                        insertSql.Append(insertValues.ToString().TrimEnd(','));
                        insertSql.Append(")");
                        insertSql.Append(";SELECT LAST_INSERT_ID()");
                    }
                    else if (this.DataBase.DBType == DataBaseType.SqlServer)
                    {
                        insertSql.Append(insertFields.ToString().TrimEnd(','));
                        insertSql.Append(" ) VALUES ( ");
                        insertSql.Append(insertValues.ToString().TrimEnd(','));
                    ...
                    else if (this.DataBase.DBType == DataBaseType.Oracle)
                    {
                        insertFields.AppendFormat("{0},", entityInfo.Columns[entityInfo.AutoGeneratedKey]);
                        insertValues.AppendFormat("{0}.NEXTVAL,", entityInfo.SequenceName);
                        insertSql.Append(insertFields.ToString().TrimEnd(','));
                        insertSql.Append(" ) VALUES ( ");
                        insertSql.Append(insertValues.ToString().TrimEnd(','));
            ...
            if (entities != null && entities.Count > 0)
            {
                ...
                try
                {
                    ...
                    for (int b = 0; b < batch; b++)
                    {
                        ...
                        if (this.DataBase.DBType == DataBaseType.Oracle)
                        {
                            insertBatchSql.Insert(0, "BEGIN", 1);
                            insertBatchSql.Append("END;");
                        }
        ...
        protected int DeleteBatchT<T>(List<T> entities)
        {
            try
            {
                ...
                for (int b = 0; b < batch; b++)
                {
                    ...
                    if (this.DataBase.DBType == DataBaseType.Oracle)
                    {
                        deleteBatchSql.Insert(0, "BEGIN", 1);
                        deleteBatchSql.Append("END;");
                    }
        ...
        protected int UpdateBatchT<T>(List<T> entities)
        {
            try
            {
                ...
                for (int b = 0; b < batch; b++)
                {
                    ...
                    if (this.DataBase.DBType == DataBaseType.Oracle)
                    {
                        updateBatchSql.Insert(0, "BEGIN", 1);
                        updateBatchSql.Append("END;");
                    }

...
namespace DNet.Cache
{
    public class EntityInfoCache : BaseCache<Type, EntityInfo>
    {
        ...
        protected EntityInfo GetEntityInfo(Type key)
        {
            ...
            {
                //非列特性
                object[] notcolumnAttrs = property.GetCustomAttributes(typeof(NotMappedAttribute), true);
                ...
                {
                    object[] keyGeneratedAttrs = property.GetCustomAttributes(typeof(DatabaseGeneratedAttribute), true);
                    if (keyGeneratedAttrs.Length > 0)
                    {
                        bool isAutoGenerated = (((DatabaseGeneratedAttribute)keyGeneratedAttrs[0]).DatabaseGeneratedOption == DatabaseGeneratedOption.Identity);
                        if (isAutoGenerated)
                        {
